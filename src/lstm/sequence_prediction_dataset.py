...
class SequencePredictionDataset(Dataset):
    """
    encoded_syscalls örn: [w1, w2, ..., w30]

    input_ids  = [w1, w2, ..., w29]
    target_ids = [w2, w3, ..., w30]

    Böylece model her adımda bir sonraki syscall'ı tahmin eder.
    label ise anomaly evaluation için metadata olarak tutulur.
    """
...
def collate_sequence_prediction_fn(batch):
    ...
    return {
        "input_ids": padded_inputs,     # [B, L]
        "target_ids": padded_targets,   # [B, L]
        "labels": labels,               # [B]
        "seq_lens": seq_lens,           # [B]
    }
