...
def save_history(history: list[dict], output_file: str):
    ...
    print(f"Saved sequence predictor training history to: {output_file}")
...
if __name__ == "__main__":
    ...
    model_output = r"data\processed\php_cwe_434\lstm_sequence_predictor.pt"
    history_output = r"data\processed\php_cwe_434\lstm_sequence_predictor_history.csv"

    vocab_size = load_vocab_size(vocab_file)
    print(f"Vocab size: {vocab_size}")

    # İlk deneme için kontrollü subset
    train_dataset = SequencePredictionDataset(
        train_csv,
        max_rows=500_000,
        only_normal=True,
    )

    val_dataset = SequencePredictionDataset(
        val_csv,
        max_rows=20_000,
        only_normal=False,
    )
    ...
    print("\n====================")
    print("START SEQUENCE PREDICTOR TRAINING")
    print("====================")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_metrics["loss"] < best_val_loss:
            ...
            print(f"Saved best sequence predictor model to: {model_output}")
