...
class EncodedSequenceDataset(Dataset):
    def __init__(self, df: pd.DataFrame):
        ...
        self.sequences = [parse_encoded_sequence(x) for x in df["encoded_syscalls"].tolist()]
...
def build_validation_trace_df(model, validation_csv, device, batch_size=512):
    ...
    df = pd.read_csv(validation_csv, sep=";", usecols=["trace_name", "label", "encoded_syscalls"])
    ds = EncodedSequenceDataset(df)
...
def streaming_test_trace_df(model, test_csv, device, top_fraction=0.10, chunksize=50_000, batch_size=512):
    ...
    for chunk_idx, chunk in enumerate(
        pd.read_csv(test_csv, sep=";", usecols=["trace_name", "label", "encoded_syscalls"], chunksize=chunksize),
        start=1
    ):
        ds = EncodedSequenceDataset(chunk)
        dl = DataLoader(
            ds,
            batch_size=batch_size,
            shuffle=False,
        ...
        if chunk_idx % 20 == 0:
            ...
            remaining = sum(counts.values()) - total_rows
            eta_min = (remaining / rate) / 60 if rate > 0 else -1
            print(
                f"Test processed rows: {total_rows:,} / {sum(counts.values()):,} "
...
if __name__ == "__main__":
    ...
    vocab_file = base_dir / "syscall_vocab.json"
    model_file = base_dir / "lstm_sequence_predictor.pt"
    validation_csv = base_dir / "validation_encoded_syscalls.csv"
    test_csv = base_dir / "test_encoded_syscalls.csv"

    output_val = base_dir / "trace_level_syscall_validation_scores.csv"
    output_test = base_dir / "trace_level_syscall_test_scores.csv"
    output_summary = base_dir / "trace_level_syscall_eval_summary.csv"
    ...
    print("\n====================")
    print("TRACE-LEVEL SYSCALL LSTM RESULTS")
    print("====================")
    ...
    summary_df = pd.DataFrame([{
        "method": "LSTM_SYSCALL_TRACE_LEVEL",
        "top_fraction": 0.10,
